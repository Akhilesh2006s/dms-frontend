...
export default function WarehouseHoldDCScreen({ navigation }: any) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const data = await apiService.get('/dc-orders?status=dc_on_hold');
    ...
    <View style={styles.container}>
      <LinearGradient colors={gradients.primary} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerTitle}>Hold DCs</Text>
      ...
      <ScrollView style={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {dcs.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>No hold DCs found</Text>
          </View>
        ) : (
          dcs.map((dc) => (
            <View key={dc._id} style={styles.card}>
              <View style={styles.cardHeader}>
                ...
                <View style={[styles.statusBadge, { backgroundColor: colors.warning + '20' }]}>
                  <Text style={[styles.statusBadgeText, { color: colors.warning }]}>On Hold</Text>
                </View>
              </View>
              <View style={styles.cardBody}>
                ...
                {dc.remarks && (
                  <View style={styles.infoRow}>
                    ...
                    <Text style={styles.infoValue}>{dc.remarks}</Text>
                  </View>
                )}
              </View>
            </View>
