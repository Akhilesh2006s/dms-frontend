  ...
  const loadReturns = async () => {
    try {
      ...
      const filtered = list.filter((r: any) =>
        ['Received', 'Pending Manager Approval'].includes(r.status)
      );
      setReturns(filtered);
    } catch (error: any) {
    ...
    <View style={styles.container}>
      <LinearGradient colors={gradients.primary} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerTitle}>Warehouse Manager Returns</Text>
          <LogoutButton />
        </View>
      </LinearGradient>
      <ScrollView
      ...
      >
        {returns.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>No returns awaiting approval</Text>
          </View>
        ) : (
          returns.map((r) => (
